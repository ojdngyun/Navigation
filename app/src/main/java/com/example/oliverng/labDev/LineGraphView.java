...
public class LineGraphView extends View
{
     ...
     */
    public LineGraphView(Context context, int dataWidth, List<String> labels, int screenWidth) {
        super(context);
    ...
    public void setColors(int[] colors)
    {
        ...
        for(int i = 0; i < Math.min(labelSize, colorLength); i++)
    ...
    protected void onDraw(Canvas canvas){
        ...
        for(int i = 1; i < pointSize; i++){
            int pointILength = points.get(i).length;
            for(int j = 0; j < pointILength; j++){
    ...
    public void addPoint(float[] y)
    {
        ...
        if(points.size() > maxDataWidth) {
            points.remove(0);
            Log.d("dataPoint", "DataPointRemoved");
        }
